...
- [ ] **模块6 · OEM 体系**(方案定稿 2026-07-18;全貌见 CLAUDE.md §4 模块6 行 + memory oem-nexus-plan)
  ...
  - [ ] **大屏"缺数据源"清单**（2026-07-26 逐面板审计，1.6.9 已把这些从"编的数字"改成诚实的
    ...
    - [ ] **成功率**：网关目前**不记录请求成败**，抽屉里显示「—·网关未采集成功率」。
      要真值需在 `nexus/gateway.py` 逐请求记 ok/fail 并入库聚合。
      ⚠️ 注意：这类比率**永远不要落 0%**——0% 成功率读起来像"全线故障"，比不显示更糟。
    - [ ] **延迟 / 峰值 req/s**：网关没有分钟级采样，现显示「—」「峰值未采集」。
      要真值需网关记录每请求耗时 + 按分钟桶聚合。
    - [ ] **实例区域（底部 Region）**：现显示「—」。实例地域已有（`nexus_instance_geo`），
      可改成按在线实例的地域聚合显示，属小改。
    - [ ] **网关健康/探活**：底部「API Gateway」现按"有无在线实例"粗判，非真实探活。
