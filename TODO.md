...
> 更新:2026-07-27(大屏数据源补齐;iOS 端评估结论写入 F 节移动端条目)
...
- [ ] **移动端 App(iOS)** —— 路线已定:**fork Element X iOS**(负责人 2026-07-27 已把代码放进
  `GuDuu OS个人版-ios/`,并完成换标)。**负责人决定暂缓开发**,以下为 2026-07-27 读码评估结论。
  - **现状**:Element X iOS 官方客户端 fork(5505 个 Swift 文件/5GB,内嵌独立 `.git`);已换标
    (App 名/图标/Bundle `com.anqi.cosmac`/默认服务器 `test.guduuos.com`);**39 个改动未提交**。
  - **开工前必须先定的三件事**:
    - [ ] **定名**:换标用的还是**旧名 CosMac**(含 Bundle ID),而产品已更名「GuDuu OS 个人版」。
      ⚠️ Bundle ID **一旦上架就改不了**,必须先定。
    - [ ] **定域名**:App 默认连 `test.guduuos.com`,这是**第三个域名**(开发机=dev-cs/218、
      主站=正式机 8.140.250.163)。App 到底连哪台要定。
    - [ ] **法务**:Element X 是 **AGPL-3.0 / 商业双许可**。闭源分发衍生品有法律风险,
      对外上架前必须法务确认或走商业许可。**这条不清,后面全白做。**
  - 🔴 **最高优先级技术坑:E2EE 会让主 AI 直接变瞎(且"晚做有既成事实损害")**
    - Element X **新建房间默认开加密**(`isEncrypted: !appSettings.forceDisableE2EE`,该值默认 false)。
      而我们的 bot 是 appservice、`cosmac/bots/matrix_client.py` 里**加密相关代码 0 行**
      → 手机端一旦建了加密房,**AI 在那个频道彻底读不到消息**,所有 AI 功能当场失效。
    - 好消息:Element X 内置 `forceDisableE2EE` 开关(AppSettings.swift:297);
      坏消息:**开源版没有任何代码给它赋值**(`RemoteSettingsHook` 只处理 `enforceElementPro`)。
      → **解法=改一行**:默认值改 `true`,或从我们自己的 well-known 灌进去。
    - ⚠️ **必须赶在任何人用手机建房之前做掉**——已经加密的房,历史消息 AI 永远读不到,无法追溯修复。
    - 现状安全:生产 Synapse 未开默认加密,**现有房间都是明文**,AI 正常;风险只在 iOS 新建的房。
  - [ ] **`.well-known/matrix/client` 返回 404**(fork 内 COSMAC.md 已记):登录发现会失败。
    顺带:E2EE 策略也在这个文件配——**一个文件解决两件事**。
  - [ ] **推送(必做,不是可选)**:自建 **Sygnal** + 公司 **APNs 证书**(需 Account Holder 权限);
    NSE 已临时去掉 `usernotifications.filtering`(要向 Apple 申请才能加回)。
    **没推送的 IM 手机端等于没做**。
  - [ ] ⚠️ **苹果内购抽成 30%**:刚做完的 token 充值/会员订阅/创作者商城属"数字内容",
    iOS 上卖须走 IAP 抽 30%,而我们接的是支付宝/微信。
    常见解法:**App 内不放购买入口**,只显示余额、引导去 Web 充值。不提前设计会被审核拒。
  - **工作量分类**(差距量级:**103 个 `/cosmac/*` 端点 + 20+ 个 `cosmac.*` 事件类型 + 20+ 个自定义界面**,
    Element X 对这些一无所知):
    - ✅ **白拿**(约占传统 IM 开发量 70%+):登录/频道/私聊/时间线/已读/图片视频文件/语音/
      回复编辑撤回/@提及/表情回应/搜索/成员管理/多设备/离线缓存/E2EE。
      **流式回复也白拿**——它靠消息编辑(`m.replace`)实现,Element X 原生支持编辑聚合。
    - 🟡 **要开发但不难**(标准 REST + SwiftUI 页面):中枢 AI 入口(本质是跟 bot 的私聊房,几乎白拿)/
      我的AI工坊/AI Agent 商城/Token 钱包/会员·额度/创作者上架收益/图文教程。
    - 🟠 **要动 Element X 内部(侵入式,成本高)**:选择卡等**富卡渲染**(`cosmac.card`,要改 Rust SDK
      timeline)/AI 同事身份视觉区分/工作区(Space)分组导航/任务看板·数据看板嵌入导航体系。
    - ⛔ **建议不做**(留在 Web):管理后台、创作者审核、组织人事、平台知识库管理——运营场景,
      手机上做纯属浪费。
  - **建议分两期**:
    - [ ] **一期(能用,性价比最高)**:E2EE 默认关 + 补 well-known + 接推送 + 定名/定域名 +
      中枢 AI 入口 → 出来就是"能聊天、能用 AI、有推送"的可用 App。
    - [ ] **二期(好用)**:工坊/商城/钱包/任务看板等自定义页面,按使用频率排。
