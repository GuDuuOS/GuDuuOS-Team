...
## 2026-07-31 — GuDuu OS 1.6.32 (patch)

- 修复：中枢 AI 拆解任务时可能把任务派给不在该频道的真人（全局名册与频道成员不一
  致），导致任务悬空——被指派人侧边栏看不到频道、频道成员里也查无此人。现在登记/
  改派任务后服务端自动校验：真人执行者或挂名负责人不在任务所属频道时**自动邀请**
  进频道（TA 接受后即可看到频道和任务），邀请失败会带真实原因回报。
- 修复：被指派人写成裸用户名（如 duxz01，不带 @ 全 id）时，群内 @ 通知会静默跳过、
  当事人毫不知情。现自动归一成完整账号 id，通知照常送达。
- 优化：在与中枢 AI 的私聊里拆解任务时，若派给了真人，会明确提醒「该成员不会收到
  频道通知」，建议转告或改在具体频道拆解。

## 2026-07-31 — GuDuu OS 1.6.31 (patch)

- 修复（安全）：网页抓取（fetch_url）补上 DNS rebinding 防护——此前域名可先解析到
  公网通过校验、再重绑到内网/云元数据。现在建连后、读取内容前会校验实际连上的
  对端 IP，重绑目标的响应一个字节都读不到，读取型 SSRF（元数据凭据窃取）彻底堵死。
- 优化：走 HTTP(S) 代理的环境（对端是代理自身）自动豁免对端校验，避免开发机本地
  代理下所有正常抓取被误杀；生产容器不配代理，防护照常生效。

## 2026-07-31 — GuDuu OS 1.6.30 (patch)

- 修复：任务看板卡片显示的进度百分比与截止时间，此前与中枢 AI 汇报的对不上——AI
  读任务的工具没返回这两个字段，只能凭空编数字。现已让工具与看板取同一份数据，
  进度与截止时间一致。
- 修复（安全）：手动/测试支付渠道的确认签名过去会随下单响应回传给客户端，任意用户
  正常下单即可拿到本单签名、在该渠道被误开时不付款开会员/充值。现在签名只在服务端
  日志可见，绝不下发客户端。
- 修复：创作者技能「一次性买断」在并发/双击「获取」时可能被重复扣款；现按买家与技能
  维度串行化，重复请求不再重复计费。
- 修复：买断扣款成功后若因「已获取数量达上限」写入失败，会出现扣了款却没拿到技能；
  现改为扣款前先校验上限，避免钱货不一致。
- 修复：知识库/技能聊天命令在「实名频道暂时只剩 2 人」时会被误判为私聊，导致内容写进
  个人库并跳过频道管理员校验；现与主 AI 判定口径统一。
- 修复：每日免费额度的消费过去是「读取-判断-自增」三步、并发下可超发；现改为数据库
  原子的部分消费，免费额度绝不会写穿上限。
- 变更：SDK 引擎的内置联网搜索（WebSearch）拉黑——它绕过会员门控，免费用户可白嫖
  联网。联网统一走自研 web_search 工具，门槛在后台「会员权限」页可调、服务端强制
  （负责人定：免费用户没有联网功能）。
- 修复：同一模型的 Agent 实例被多个频道并发共享时，真实用量计数会互相覆盖导致
  Token 计费串号（偏多或偏少）；现改为线程本地计数，各频道各记各的账。

## 2026-07-31 — GuDuu OS 1.6.29 (patch)

- 修复：Nexus 生产大屏在未授权、令牌失效、接口故障或真实 OEM 数据为空时，不再静默
  展示内置的虚构实例、Token 与收入数据；现在分别显示授权、故障和真实空状态。
- 变更：虚构数据只允许通过地址显式添加 `?demo=1` 进入演示模式，并持续显示
  “演示数据 · 非生产”标识，避免演示内容被误认为线上运营数据。
- 优化：接口故障页会说明错误原因和上次成功更新时间，失效的大屏令牌会立即清除，并
  提供返回控制台及重新加载入口；无需数据库迁移。

## 2026-07-31 — GuDuu OS 1.6.28 (patch)

- 修复：同一用户过去可从多个频道同时发起 AI 请求，一起穿过“回复前检查、回复后结算”
  的配额与 Token 前拦。现在同一用户的完整 AI 回复周期会跨频道串行，不同用户仍可并发。
- 修复：日/月配额计数改为数据库原子建行、原子自增和带上限消费；多人/多线程争用最后
  一次额度时只有一个请求能成功拿到，不再因读改写覆盖而漏计或写穿上限。
- 修复：同一 Nexus OEM 实例过去可并发提交多条长模型请求，在旧余额为正时全部获准并
  形成大额透支。现在同实例只允许一条在途请求，后续请求返回 429，完成后可安全重试。
- 修复：Nexus 充值与扣费改为数据库原子加减，支付回调和网关结算并发时不再丢失任一笔
  余额变化；无需数据库迁移。

## 2026-07-30 — GuDuu OS 1.6.27 (patch)

- 修复：Nexus 运营大屏把实例域名、模型名和流水说明直接拼入 `innerHTML`，恶意或被入侵
  的 OEM 实例可借构造域名在管理员浏览器执行脚本。现在所有接口动态字段均通过 DOM
  文本节点渲染，实例标识走 `dataset`，颜色也限制为安全的十六进制值。
- 修复：大屏接口过去同时接受只读令牌和管理员令牌，控制台还会把管理员令牌带进大屏；
  一旦展示页发生 XSS，风险会从“读取运营数据”扩大到签发 KEY、充值等管理权限。现在
  大屏接口只接受只读令牌，管理员控制台通过受保护端点兑换只读凭据后再进入大屏。
- 变更：大屏令牌和 Nexus 控制台登录态从长期 `localStorage` 改为当前标签页的
  `sessionStorage`，旧数据首次访问时自动迁移并删除；令牌 URL 片段加载后立即从地址栏
  清除。关闭标签页后需要重新从安全链接或控制台进入，这是主动缩短凭据暴露时间。
- 优化：Nexus 大屏新增严格内容安全策略，只允许同源脚本，并补齐禁止套壳、MIME 猜测、
  跨站来源泄露和浏览器敏感权限等响应头；新增 XSS 渲染、最小权限和安全头回归测试。
...
- 修复：正式站新增两个精确匹配端点；客户端发现返回公开 API 地址并带跨域许可，联邦
  发现明确委派到真实 Synapse `server_name`，避免把公开访问别名误当成实际账号域名。
- 优化：发行版增加宿主 nginx 配置模板及反代说明；`doctor.sh` 新增客户端发现检查，
  同时验证 JSON 关键字段和 CORS，联邦检查也从“仅看 HTTP 200”升级为验证 `m.server`。

## 2026-07-30 — GuDuu OS 1.6.25 (patch)

- 修复：图文教程里的 Markdown 图片过去会直接加载任意外站地址，恶意作者可放置一张
  不可见的追踪图，在读者打开文档时获取其 IP、访问时间与浏览器信息。现在外部图片默认
  只显示保护提示，不产生任何图片请求；读者需要点「加载外部图片（将连接外站）」才会加载。
- 优化：用户明确允许后也只加载 HTTPS 图片，并通过 `no-referrer` 隐藏当前文档地址；
  明文 HTTP 图片始终禁止加载，切换文档、进入编辑、取消或保存后都会恢复默认保护状态。
- 修复：历史接口允许文档封面直接使用 http(s) 外链，存在同类追踪风险；网页端现在只显示
  上传到 Matrix 媒体库的 `mxc://` 封面。旧外链数据不会删除，管理员可重新上传安全封面。
- 不受影响：聊天图片/视频/文件和新上传的教程封面继续走 Matrix 媒体流程，正常显示。

## 2026-07-30 — GuDuu OS 1.6.24 (patch)

- 修复：网站此前只配置了 HTTPS/HSTS，缺少内容安全策略和基础浏览器安全响应头；
  如果页面未来出现脚本注入，浏览器没有第二层来源限制，登录态与聊天数据面临更高风险。
  现在新增强制 CSP，并禁止第三方页面套壳、MIME 猜测与完整跨站来源泄露。
- 优化：安全策略兼容现有 Matrix WebSocket、WASM 加密、媒体附件、字体 CDN、
  Web Worker 与 Cloudflare Turnstile，不影响登录和聊天的正常资源加载。
- 变更：直出、宿主反代和旧版升级三条部署路径共用同一份安全配置；旧安装升级时只向
  已生成的 Caddyfile 幂等插入引用，不覆盖 OEM 自己调整过的反代规则；重启前先完整
  校验网关配置，语法错误会中止升级而不会替换仍在服务的旧容器。

## 2026-07-27 — GuDuu OS 1.6.23 (patch)

- 优化：中枢 AI 右侧「进度」的计数，**没有任务时显示「—」而不是「0/0」**。
  负责人明确否掉了「没任务就隐藏计数」的方案——「哪怕只有 1 个任务也需要看到」，
  所以计数元素**位置保留、不隐藏**，有任务时照常显示 `1/1`；只是 `0/0` 这个数字
  在空状态下没有意义、还容易被当成出错。

## 2026-07-27 — GuDuu OS 1.6.22 (patch)

- 变更：**用户交代的每件事都落成任务，哪怕只有一步**（负责人：「他不需要拆任务，
  但这个本身就是一个任务，所以至少一个任务」）。此前 AI 只在判断「需要拆解」时才调
  `create_tasks`，于是「帮我装个技能」这类单步请求做完就无影无踪，右侧进度面板一直空着。
  - 行为准则新增第 8 条 + 改写 `create_tasks` 的工具描述，两处都点明「可以只有一条」。
  - 同时立了两条边界，避免矫枉过正：
    · **做完必须 `update_task` 标 done** —— 建了不更新，面板会永远挂着「进行中」，比不建更糟；
    · **纯聊天/问答/查询不建**（「今天几号」「解释下这段代码」这类没有交付物的），
      否则看板会被噪音刷满。
  - 这是 prompt 层的行为引导，不改架构。效果取决于模型的遵循程度，需线上实际观察。
...
- 新增：**导入前的依赖缺口提示**。manifest 里的 `skill_slugs` / `workflow_slugs`
  若引用了用户手上没有的资源，注入侧是**静默跳过**的（资源被删不该让整条回复崩，
  这本身对）——副作用是用户装了个引用 3 个技能、实际一个都没有的 Agent 却毫不知情，
  只觉得"这 Agent 怎么不好使"。现在预览阶段就把缺口挑明：
  - 工坊导入弹窗：确认前显示「⚠️ 缺依赖：它引用了技能 X、工作流 Y，而你手上没有」。
  - 主 AI 路径：预览结果里带同样的缺口，并要求 AI 如实转告用户再让他决定。
  - 判定口径：用户**可见的**全局技能 + 他自己的个人技能都算"有"，避免误报吓唬人。
- 优化：**安装成功后的用法引导**。此前只说"可在工坊查看或删除"，用户不知道接下来怎么用。
  现在明确告知三件事：已进入主 AI 派单名册（显示「我的·<名称>」）、
  **在任意频道直接输入它的名字即可点名**（不需要额外绑定频道）、改人设去工坊。
  （核实过 `_match_routable_agent`：候选本就包含"发起人自建"，个人智能体确实无需绑频道。）
- 测试：新增 4 条（缺口检测、个人技能算"有"、无引用不误报、检查出错不阻断导入）；
  全套 836 通过。

## 2026-07-27 — GuDuu OS 1.6.17 (patch)

- 修复：**AI 建群时会把已停用账号也邀进去**（负责人 dev 实报）。排查发现校验有缺口——
  `assemble_team`（一键建专班）早就过滤停用账号了，但 **`create_room`（建群）这条路径的
  `invitees` 直接透传、完全没校验**，AI 从这条路建频道就会邀到登不进来的人，成员列表里
  留一个永远「待接受」的死条目。现已与 assemble_team 对齐：过滤停用、并在回复里如实
  说明跳过了谁（不静默吞掉）。**发起人永不过滤**——他是群主，滤掉会建出一个自己都不在的房；
  查不到停用状态时 fail-open 不过滤，免得一次查询抖动就建不成群。
- 增强：**成员列表标注「已停用」**。工具层拦得住真正的邀请动作，却拦不住 AI 在自由回复里
  「建议」拉某人入群——而它推荐的人可能早被停用（负责人截图里正是这种）。把状态直接摆进
  它看得到的数据里，它才不会推荐一个登不进来的人。
- 排查副产品（已核实、非 bug）：单个邀请的 `_invite_precheck` 三种拦截（自己/已在群/停用）
  都正常；停用检测本身也正常（本产品「停用」= Synapse 的 `locked`，实测能正确识别）。
  AI 之所以还会「说」出停用账号的名字，是因为那是自由文本建议、名字来自对话历史，
  并非名册——名册里查无此人（该用户的个人名册没有他，全局名册为空）。
- 新增回归测试 `cosmac/tests/test_invite_guard.py`（9 项）：建群过滤/发起人豁免/fail-open/
  三种邀请拦截/成员列表标注。

## 2026-07-27 — GuDuu OS 1.6.16 (patch)

- 修复：**登录页切到「账号登录」后输入框仍显示上一个 Tab 的邮箱**（负责人在 dev 环境实报，
  且是二次反馈）。排查后确认**不是 Vue 状态残留**——线上构建已带 `key="login-account"`/
  `key="login-email"`（两个 tab 是独立元素、绑不同变量），代码里也没有任何地方把邮箱写进
  账号变量。真因是**浏览器自动填充**：账号框带 `autocomplete="username"`，而浏览器为本站
  保存的"用户名"就是用户那个邮箱（他一直用邮箱 tab 登录），新元素一渲染就被填回去。
- 处置（不跟自动填充对着干——硬压会连带弄坏密码管理器）：
  - **让「账号登录」也认邮箱**：输入里含 `@`（且不在首位）就自动走邮箱登录通道。
    浏览器填什么都能登进去，而不是撞一句语义模糊的「用户名或密码错误」。
    占位符与空值提示同步改为「用户名或邮箱」。
  - 切 Tab 时**补清两个输入框**（原先只清了密码和报错）——状态层本该清干净。
  - 「重新发送验证码」走同一套判断，避免账号框填邮箱时重发走错通道。
- 边界：判定用 `indexOf('@') > 0` 而非 `includes('@')`——**Matrix 用户 ID 本身以 @ 开头**
  （`@user:domain`），用 includes 会把粘贴完整 MXID 的人误判成邮箱登录。

## 2026-07-27 — GuDuu OS 1.6.15 (patch)

- 新增：**AI 会话标题自动概括**。左侧会话列表此前是「首句砍前 24 字」，用户粘个 GitHub
  链接进来，列表里就是一串 URL，看不出这段在干嘛（负责人实报）。现在**首轮回复之后**
  让模型概括一句（≤12 字）写进 `cosmac.ai_session` 标记，前端优先读它。
  - 只在首轮做一次：标题是"这段会话干嘛的"，不该随聊天漂移，也免得每轮多烧一次调用。
  - 写既有的会话标记而非 `m.room.name`——改房名会影响 Element 等其它客户端的显示。
  - 全链路 fail-safe：模型失败/无权限/老会话一律回退原来的截断逻辑，且**放在回复发出
    之后**执行，绝不挡在用户看到回复的前面。
- 新增：**导入内容的来源标记**。Skill/Agent 各加 `source_url`：空=自建，非空=从该地址
  导入。工坊列表上标一枚「导入」徽章，悬停显示出处。装久了才分得清哪些是自己写的、
  哪些是从网上装的；第三方内容会注入进模型上下文，**可追溯是底线**。
  - 记的是用户粘的**原始地址**而非规整后的 CDN 直链——用户认得出自己粘了什么，
    日后核对/重装也是回 GitHub 那个页面。
  - 每次保存都写，所以「从导入改成手写」后标记会自然消失。
  - 同样走 `_heal_business_schema` 自愈补列（两张表各加一列），生产升级无需手工介入。
- 测试：新增 8 条（标题的首轮只做一次/引号截断/模型失败静默/非会话房不碰、
  来源字段往返与自建为空）；全套 823 通过。
...
- 新增：**从 GitHub 导入 Skill / Agent**（负责人要的「别人 GitHub 上的能不能装进来」）。
  此前没有任何导入通道，用户让主 AI 去装，AI 只能拿 Claude Code 那套 `~/.claude/skills/`
  硬凑（正是 1.6.12 修掉的那次事故）。现在给了正规通道：工坊「我的智能体 / 我的技能」页
  各有「从链接导入」，粘 manifest 地址 → **预览** → 读完全文 → 确认才落库。
  - **manifest 是一份 JSON**（`kind/slug/name/description` + 技能的 `instructions` 或
    智能体的 `system_prompt`/`model`/`skill_slugs`/`workflow_slugs`）。**只搬数据，
    绝不下载压缩包、不解压、不执行任何代码**——装第三方 Agent 的价值在它的人设与配置。
  - **强制两步走**：第三方人设会作为系统提示注入给主 AI，而主 AI 手里有建群/发消息/
    查知识库等工具，静默导入等于把一部分控制权交给素未谋面的作者。所以预览必须展示
    **人设/正文全文** + 风险提醒（提示通读、找「忽略以上指令」这类可疑句）。
  - **内容哈希锁定防 TOCTOU**：预览返回 sha256，确认时**重新拉取并比对**——挡住
    「预览时良性、你读文本那几十秒里把仓库文件换成恶意版本」。不一致返回 409 让用户重看。
  - **SSRF 双保险**：URL 是用户给的，服务端拿它外呼是典型 SSRF 面。先过**域名白名单**
    （GitHub/Gist，白名单比黑名单安全，且不受 `COSMAC_WF_ALLOW_INTERNAL` 开关影响），
    再过 `wf.check_outbound_url`；且 `allow_redirects=False`（否则 302 到内网即绕过）。
    另有 256KB 上限、15s 超时、只允许 https。
  - **不采信 manifest 里的平台侧字段**：`access`/`enabled`/`scope` 等一律丢弃，
    否则等于让第三方作者自己声明「我对所有人可见」。**不递归导入**引用的技能/工作流——
    一次点击拉进一整棵未审阅的依赖树，用户根本不知道自己装了什么。
  - 落库复用工坊既有的保存端点，门控/条数上限/存储配额全走同一套，
    避免导入成为绕过配额与门控的后门。
  - 新增 16 条测试（白名单/协议/重定向/超大/非 JSON/字段越权/slug 规范/去重截断）。
- 修复：工坊导入面板文案里的 Markdown 星号被原样显示（`**定义**` → `<b>`）。
...
- 新增：**智能体可以绑定工作流**——让 Agent 从「只会说话」变成「能真的干活」。
  起因是负责人指出工坊不像 n8n/Coze/Make 那样能接外部能力。排查后确认差距不在「没对接」
  （n8n/Make/Coze/Dify/ComfyUI 早在模块3 就接好了，SSRF 防护/凭据绑域名/异步回调都齐），
  而在**两个世界是割裂的**：工作流只有管理员配得了，且 Agent 压根没有绑定工作流的字段——
  一个 Agent = 人设文本 + 技能文本，全是往 prompt 里塞字符串。现在两边打通：
  - **后台全局智能体**（管理员配）：勾选绑定工作流 → 绑了该智能体的频道里，AI 知道有这些
    工作流并能直接调用。**绑定即授权**：这批在该频道免受 `workflow_run` 会员门控——
    管理员显式配置本身就是授权，再让它撞门槛会出现「AI 说能用、一调就被拒」的荒谬体验
    （与既有的「群绑定技能不按发起人过滤」同一原则）。
  - **前台工坊**（用户自建）：同样能勾选绑定。但**个人绑定不构成授权**，只用于告诉 AI
    「该用哪些」，能不能跑仍由门控裁决——否则任何付费用户自建一个 Agent 绑上去就绕过了
    门槛 = 自助提权。UI 上如实写明，不给错误预期。
  - 新增 `GET /cosmac/my/workflows`：工坊勾选需要清单，但连接器定义存控制室、普通用户
    读不到 → 由 bot 代读并**裁剪**，只下发 slug/name/输入提示。**url（常是内网 webhook）、
    凭据名、graph 一律不出网**——顺手回传整条定义等于把内网地址广播给所有登录用户。
  - 豁免口径收得很紧：只对 `run_workflow` 生效、slug 必须精确命中、不带 slug（列清单）
    不豁免、豁免的只是会员门控（用量配额照扣、SSRF/凭据校验一步不少）。
- 变更：`cosmac_agent` 表新增 `workflow_slugs`(JSON)。本项目没上 alembic、`create_all`
  不给旧表补列 → 已在 `engine._heal_business_schema` 加自愈 `ALTER`（默认 `'[]'`，
  已有行自动成为"没绑任何工作流"），沿用该函数既有惯例，**生产升级无需手工介入**。
- 测试：新增 10 条，其中 4 条是安全断言——个人绑定后仍被门控拦下（不提权）、豁免不外溢到
  其它工具、可绑定清单只含三个键且断言 url/凭据名不出现在响应里、`ToolContext` 默认值
  必须是不可变空元组（dataclass 用可变默认值会让所有实例共享同一个 list）。全套 788 通过。

## 2026-07-27 — GuDuu OS 1.6.10 (patch)

- 新增：**网关请求质量真实采集**（补上 1.6.9 记的「大屏缺数据源清单」里的成功率/延迟/峰值三项）。
  网关此前不记请求成败与耗时，大屏只能显示占位「—」。现在转发前后打点、**成败都记**（含上游
  不可达的 502），按**分钟桶**聚合进新表 `nexus_request_stat`（实例×分钟为复合主键，每实例
  每天最多 1440 行，还天然给出「峰值 req/min」）。不能复用 ledger——它只在扣费成功时写，
  失败请求毫无留痕，成功率无从算起。统计是 best-effort，绝不影响转发与计费。
  口径延续 1.6.9：今日无请求一律返回 null、前端显示「—」，**不落 0%**。
- 新增：**地图标记显示区域与节点数，多台可点开区域节点列表**。同地域的多台实例在地图上合并成
  一个标记（否则叠成重影），而合并后取的是首台 id——**除首台外的机器既不显示也点不到**，
  等于在界面上不存在。现在：标记始终显示节点数（如「浙江 · 3 个节点 · 1/3 在线」，原先只有
  多于 1 台才显示台数，看到「浙江」根本不知道是几台）；多台时点标记先出列表（区域名 + 节点数 +
  在线数 + 累计 Token，逐行为状态点/名称/代号/状态/Token/成功率，按 Token 降序），点某行
  再进那台详情。单台不弹列表、直接进详情。
- 修复：**大显示器上文字发虚**。排查确认不是渲染问题（页面无整体缩放、不存在位图拉伸），
  而是视觉规格：242 个文字元素里 **80% 字号 ≤10px（最小 7px）、67% 对比度低于 AA**。
  小字配浅灰在高像素密度的笔记本屏上是「精致」，换到大显示器后像素密度减半（27 吋 2K 约
  109 PPI），可用像素骤减、边缘锯齿显现，观感就是「糊」。三处修复：
  - **整体自适应缩放**：以 1600px 为设计基准，视口越宽等比放大越多，封顶 2 倍。用 `zoom`
    而非 `transform: scale`——前者是布局级缩放，浏览器按放大后尺寸重新排版并重新栅格化文字，
    任何倍率都清晰；后者拉伸已画好的位图，反而更糊。
  - **对比度达标**：达标率 **33% → 99.6%**，平均对比度 7.40。按「保留层次梯度」映射到
    4.5–6.3 区间，而非全部压到同一条线（那样主/次/辅助三层层次会消失）。品牌色文字按同色相
    压暗，**色块仍用原色**（`--geo-color` 同时用于图标渐变与文字，必须拆开）。
  - **canvas 渲染倍率上限 1.5 → 2**：高清屏像素比正是 2，卡在 1.5 等于用 1.5 倍画完拉到
    2 倍显示，既少像素又因非整数倍而网格错位。已在 dpr=2 真机复验倍率为 2.0。
- 修复：**详情抽屉在演示数据下必崩**。1.6.9 接成功率真值时只判了 `=== null`，而演示节点
  根本没有 `successPct` 字段（值是 `undefined`），直接走进 `undefined.toFixed()` 抛
  TypeError、抽屉再也打不开。改用 `Number.isFinite`。此前被「点标记不开抽屉」掩盖着。
- 变更：单台地图标记点击行为——原先只「选中」（更新右侧概览），现在直接打开详情抽屉。
  既然多台那条路径点完能直达详情，单台没理由更绕。
- 修复：浮层不跟随整体缩放。详情抽屉挂在 body 下拿不到 `.app-shell` 的 zoom，加了自适应
  缩放后它比周围小一圈。⚠️ `zoom` 必须加在**容器**而非面板：面板是 `absolute right:0`，
  在它自身 zoom 会把「位置」也放大，右边缘被推出视口（实测 1920 下推到 2397、整个不可见）。

## 2026-07-26 — GuDuu OS 1.6.9 (patch)

- 变更：**大屏逐面板审计，把"编出来但看着像真的"数字全部落成诚实值**（负责人定：没有真实
  数据源的就显示 0 这类，缺的数据源另记 TODO）：
  - **营收轮播页**几乎全是写死的演示值（`$12,846` 今日收入 / `$386.2K` 本月预估 /
    `目标达成 81%` / `峰值 $1,184/h` / `14.6%` 环比）——这些元素**连 id 都没有**，JS 根本
    够不着，永远改不掉。已补 id 并落成 `¥ 0` +「支付未接入」。真钱接通后换真实订单金额。
  - **节点抽屉的成功率**此前按在线状态硬编码三档（99.97/99.82/97.82），而**网关根本不记录
    请求成败**——纯属虚构，偏偏又是最容易被当真的运维指标。改为「—·网关未采集成功率」。
    ⚠️ 特意**没有落成 0%**：0% 成功率读起来像"全线故障"，比不显示更误导（比率型指标通例）。
  - **峰值 req/s** 此前是 `今日用量 × 1260` 凭空乘出来的 → 改「峰值未采集」。
  - **底部状态栏** `Region CN-East-01` 写死 → 「—」；`API Gateway` 改按在线实例粗判。
- 修复：节点抽屉的 Token 单位**写死 `B`**，而全局单位是自适应 K/M/B——余额 100M 会被显示成
  `100.00B`（**差 1000 倍**）。改为跟随 `TOKEN_UNIT.label`。
- 优化：底部 GATEWAY CALLS 柱状图从 **sin 波装饰**（72 根假波形）换成**真实的 24 小时逐时消耗**
  （这份数据本来就有、只是没接上；此前是"数字真、波形假"）。
- TODO：新增「大屏缺数据源清单」——收入（明天随支付做）/ 成功率 / 延迟·峰值 req/s /
  实例区域 / 网关真实探活，逐条写明要在哪补采集。

## 2026-07-26 — GuDuu OS 1.6.8 (patch)

- 修复：**大屏地图上"点亮了一大片"**（负责人实报："不应该就是 1 个或 2 个吗，同一区域不应该是一个？"
  ——问得对，之前是我做错了）：
  - **一个实例点亮一片** → 我沿用了演示配色里的 `radius: 58`，那本是给空场景看的装饰"光晕"
    尺寸，用来表示真实节点就成了几十个格子糊成一团。改为**精确点亮单个格子**。
  - **同地域多台机器会重影** → 现在**按地域合并**：同一地域只出一个点，卡片显示
    「浙江 · 2 台 / dev-cs +1」并汇总用量。
  - **点位准不准** → 把经纬度**吸附到最近的陆地格子**（负责人提的"要不要先标注地图"正是这个
    顾虑）：地图是六边形栅格，投影点未必落在格子中心、沿海城市还可能落到海里；吸附后
    ①点一定在陆地上 ②标记与被点亮的格子严格对齐 ③已占用的格子会跳过，相邻地域不抢同一格。
  - 本地实测：造 3 台（2 台浙江 + 1 台广东）→ 地图上**只有 2 个点**，位置正确。
- 修复：标记的像素定位改用 **ResizeObserver**（元素真正拿到尺寸时才回调），取代之前赌时机的
  rAF——首屏有揭示动画，布局稳定的时刻不固定；顺带把窗口缩放、面板折叠也一并覆盖。
- 排查笔记（**避免下次再被骗**）：调试时一度以为 rAF/ResizeObserver 都"不生效"，实际是
  **浏览器面板处于隐藏状态**（`document.visibilityState === "hidden"`）——隐藏页面里
  rAF 与 ResizeObserver **根本不派发**（实测 500ms 内 rAF 执行 0 次），而 `dispatchEvent(resize)`
  是同步派发所以照常工作。另外本地起 nexus 复测时踩到：旧进程仍攥着**已被 rm 的旧 DB
  文件句柄**（Unix 只解链接）、新进程端口被占即退，于是一直看到上一批数据——先 kill 干净再测。
...
- 修复：**大屏地图上看不到任何节点**（负责人实报，截图为证）。根因比预想的更深——
  「全球地图」视图**从来就没接过真实数据**：热点坐标全部来自写死的 `MAP_HOTSPOT_DEFINITIONS`
  （芝加哥/柏林等演示点），节点标记 `.geo-node` 也是写死在 index.html 里的演示件
  （Nebula One / 深蓝数据…）。真实数据接管后这些演示件 id 对不上、被可见性逻辑判成
  "不在范围"而全部隐藏 → 地图上一个节点都不剩。（1.6.5 我改的投影只作用于「3D 星系」视图，
  对地图视图无效。）**修**：热点改由**真实实例的经纬度**生成、地图标记按真实节点动态重建；
  一个实例都没定位到时才回落演示坐标（免得地图全空）。
- 修复：**没填地域的实例被画到几内亚湾**。`Number(null)` 得 `0` 而**不是** NaN，于是
  `lat/lon` 为空的实例被当成经纬度 (0,0)——大西洋上那个著名的 Null Island。改为先显式
  排除 null/undefined 再判有限性；未定位的实例**不在地图上画**（宁可不画，绝不假装它在某处）。
...
- 新增：**运营大屏按真实地域给实例打点**（模块6）。此前大屏节点位置是**伪随机坐标**、
  `region` 字段填的其实是域名，地图看着有点其实全是假的。现在：
  - 母舰新增 `nexus_instance_geo` 表（**独立表**——母舰没有迁移框架，给现存表加列不会
    自动 ALTER，沿用 `nexus_key_claim` 那套零迁移风险的做法）+ `nexus/geo.py` 地域字典
    （中国 34 省级 + 港澳台 + 12 个常见境外部署地，含省会/主要城市经纬度）。
  - **地域由 OEM 装机时选**（`install.sh` 新增交互 + `--region` 参数，兑码时随请求上报），
    console 可改（`POST /nexus/admin/instance_region`、`GET /nexus/admin/regions` 取字典）。
  - 大屏改用**等距圆柱投影**把经纬度换成地图坐标；没填地域的实例回落到"按域名哈希的稳定
    伪随机位"并标 `geoKnown=false`——**绝不把"没数据"画成"在某地"**。
  - 心跳记录来源 IP，但**只存脱敏网段**（IPv4 /24、IPv6 /64）、**只供人工核对**，不参与定位。
- 决策说明（**评估后否掉了 IP 自动定位**，别再走回头路）：① OEM 实例全跑在云服务器上，
  而云 IP 的地理归属常是「服务商注册地」而非机房所在地——IP 定位恰在本场景最不准；
  ② ip2region 数据文件已换 v3 格式（`ip2region_v4.xdb`），手写二进制解析一旦解错是**静默
  出错**（点全飘还没人发现）；③ 换 MaxMind 又要额外注册领 license key。OEM 自己选一次，
  零依赖、100% 准、还没有存客户 IP 的隐私负担。
- 修复：IPv6 脱敏若手工切字符串，`::` 压缩会把末组误当第 4 组、算出**完全不同的网段**；
  改用标准库 `ipaddress` 计算（单测已覆盖）。

## 2026-07-26 — GuDuu OS 1.6.4 (patch)

- 优化：**AI 回复改为流式输出**（边生成边显示，不再憋到最后一次性发）。用户约 1 秒内就能
  看到字在往外冒，体感提速远超「长连接复用」那 2.6 秒，且完全不碰规则注入/会话隔离。
  - SDK 引擎开 `include_partial_messages`，只取 `text_delta` 增量；`thinking_delta`（模型
    内部推理）**绝不外流**。多轮工具循环里每遇到新文本块就重置草稿，与「最后一段文本即
    最终回复」的既有口径一致。
  - `_StreamWriter`：首个正文片段发一条草稿消息，之后**节流**原地编辑（1.2 秒 / 24 字 /
    单条最多 24 次编辑——每次编辑都是永久留存的 Matrix 事件，太密会刷爆时间线与限频）。
    最后把草稿定格成最终回复并告知调用方「已发过」，避免同一段话出现两条。
  - AI 同事（傀儡）回复时草稿也以它的身份发/改：新增 `edit_text_as`；身份解析前移到引擎
    开跑前，避免「草稿主AI发、定稿该同事发」的错位。
  - **全程 best-effort**：发/编辑/定格任一步失败即停用流式并如实回落到原来的一次性发送，
    绝不出现「只看到半截话」。一键停用：`COSMAC_STREAM_REPLY=0`（改 env 重启，无需回滚）。
- 调查结论（**推翻 TODO 旧前提，别再按老思路开工**）：原计划的「ClaudeSDKClient 长连接
  复用提速」经生产实测**不划算且有安全雷**——① 可省的固定开销只有约 2.6 秒（connect 0.7s
  + 首次 query 暖机 1.9s），不是注释里估的 3~8 秒；② **`query(session_id=...)` 根本不隔离
  上下文**，同一 client 上 A 会话存的内容 B 会话读得到 → 复用即**跨用户泄露**；③
  `system_prompt` 只能 connect 时定而我们每轮都不同，复用需把规则挪出 system 位、有规则
  遵守度回归风险。已把数据与结论钉进 TODO / engine.py 注释 / 长期记忆。

## 2026-07-26 — GuDuu OS 1.6.3 (patch)

- 新增：**创作者技能上架售卖（Token 经济 P4，一次性买断）**——创作者商城补齐最后一类资源：
  - **计费=买断**（本次定的口径）：技能是每轮对话**自动注入**的，按次扣用户无法预期花费；
    故改为**获取那一刻付清、之后永久可用**。与 Agent 的按次计费并存，商城卡片分别标
    「N token 买断」/「N token/次」。
  - **先付后得**：余额不足时购买**明确失败、分文不扣、不给货**（与 Agent 后付"扣到 0 为止"
    的语义刻意不同）；前端点「购买」有二次确认，防误点花钱。
  - **买断即永久**：移除后重新获取**不再重复扣款**（用收益流水反查是否付过，不另建购买表）。
  - **真正生效**：买断的技能进入买家每轮对话的 system 注入（与自建技能同权重）；创作者
    下架/删除/停用即失效；未购买者拿不到（本地实测三种情形全验）。
  - 分成同 Agent：平台抽成 10%、创作者得 90%，实时充钱包 + 逐笔进收益账本。
  - 上架同样走 **P3 审核**（提交→待审→通过才在售），管理员审核页可看**技能正文全文**。
  - 工坊上架表单加「智能体（按次）/ 技能（买断）」类型选择，列表按类型显示徽章与计价口径。
- 说明：MarketListing 加 `kind` 列区分两类；唯一键沿用 (creator, slug) 不做生产改约束手术——
  同一创作者用同名 slug 上架另一类资源时显式提示改标识（撞名极少见，换零迁移风险）。
- 本地实测：alice 建技能→上架 500 买断→管理员通过→bob 购买（扣500/抽50/alice+450）→
  移除重获取不重复扣→技能注入 bob 的对话生效、carol 无、创作者下架后即失效。767 单测全绿。

## 2026-07-26 — GuDuu OS 1.6.2 (patch)

- 新增：**创作者认证 + 上架审核（Token 经济 P3，类公众号流程，负责人定稿）**——把 P2 的
  "创作者会员即可直接上架"收紧为正规双审核闭环：
  - **创作者认证申请**：工坊「上架·收益」页非创作者看到申请入口——提交资料（称呼/联系方式/
    自我介绍/作品证明）→ 支付认证费（金额后台可配 `creator_cert_fee_cents`，默认 300 元；
    复用订单系统 kind=creator_cert，测试通道先行、真实支付接好自动切）→ 管理员审核通过=
    授予「创作者会员」资格（永久）。**拒绝不退费、可免费修改重提**（定稿口径；重提判据=
    是否已付过费）。状态机 待付费→待审核→通过/拒绝，幂等回调、入账失败自动回滚订单。
  - **上架内容审核**：**任何上架/修改（含改价/改文案/重新上架）一律进「待审」**，审核通过
    才在售；待审/被拒期间不可被购买使用（路由与计费只认 status=on）。拒绝必须写原因、
    展示给创作者，可修改后重新提交。管理员审核时可见 Agent 人设全文（审核依据；普通商城
    目录仍绝不下发人设）。
  - **后台新页「创作者审核」**：认证申请（资料全文+已付费标记+通过/拒绝）与上架审核
    （文案/价格/人设折叠查看+通过/拒绝）两块，侧栏带待审数角标。
- 本地实测通过全流程：bob 提交申请→付 300 元(测试通道)→管理员通过→bob 变创作者→上架进
  待审→管理员拒绝(带原因)→bob 看到原因改后重提→管理员通过→商城在售；存量在售条目不受影响。

## 2026-07-26 — GuDuu OS 1.6.1 (patch)

- 新增：**创作者商城上线（Token 经济 P2）**——创作者把自建 Agent 上架赚 token，闭环全通：
  - **上架**：工坊新页签「上架·收益」——创作者会员选自己的智能体、定「每次使用价」（token/次，
    0=免费）一键上架；上架是**引用**（人设仍归创作者、绝不进商城橱窗，改人设全网生效）；
    重复上架=更新价格；可自行下架/重新上架，管理员可强制下架（banned，创作者不可自行复活）。
  - **商城**：新分类「创作者 Agent」——卡片标**按次价**与创作者；获取免费（照占获取配额），
    钱在**使用时**收。
  - **按次扣费**：在频道 @创作者 Agent 即由它应答；每次回复按定价**硬前拦**（余额不够这一次
    明确拒绝，不产生欠账）→ 回复成功后扣款分账。**二选一计费**（定稿口径）：用创作者 Agent
    走固定价、不再叠加平台真实用量扣。创作者本人与管理员使用免费。
  - **分成 90/10**：每次付费使用，平台抽成 10%（后台 `platform_fee_pct` 可调、向上取整），
    其余**实时充进创作者 token 钱包**；逐笔进收益账本（`cosmac_creator_earning`，可对账）。
  - **收益可见**：工坊「上架·收益」页——累计收益/使用次数/在售状态/最近明细（本期只做账本
    可见，真实出金按定稿延后）。
  - 全程受 Token 经济总开关门控（关=创作者 Agent 免费用、不扣不分成），计费层异常 fail-open。
- 本地实测通过全链路：alice 上架 300/次 → bob 商城获取 → 频道 @文案高手 → bob 扣 300、
  平台抽 30、alice 钱包 +270、账本 +1 笔；工坊/商城 UI 同步正确。

## 2026-07-26 — GuDuu OS 1.6.0 (minor)

- 新增：**Token 经济 P1 上线（用户钱包 + 真实计量 + 充值 + 后台管理）**——模块4 变现主线的
  大功能第一片（负责人 2026-07-26 定稿：创作者 Agent 固定价/次 + 平台 AI 按真实用量扣、
  售价固定汇率+倍率后台可调、中心商城供货+各 OEM 实例销售、本期只做收益账本不出金）：
  - **用户 token 钱包**：`cosmac_token_wallet`（余额）+ `cosmac_token_ledger`（流水账，一笔一行
    只增不改、含余额快照，"体现"的底层账本）；扣费用**带余额守卫的原子 UPDATE**，并发不扣穿。
  - **真实计量**：LLM 真实用量全链路打通——SDK 引擎取 `ResultMessage.usage`、openai 兼容/Claude
    后端取响应 `usage`，`TurnResult.usage_tokens` 厂商中立贯通；回复成功后按 真实 token×倍率 结算。
  - **免费层**（负责人定）：每天送免费 token（后台可配、当天清零不累积，走 cosmac_usage 天键）；
    扣费两级：先吃今日免费额度、再扣钱包余额（扣到 0 为止，不失败已发出的回复）。
  - **充值**：token 充值包（控制室 `cosmac.token_config` 的 packages，后台配"多少钱多少 token"）；
    订单复用交易系统骨架（`cosmac_order` 加 kind/tokens 列，支付回调幂等 CAS，入账失败自动回滚
    订单待重试）；当前测试通道，真实支付宝/微信接入后即收真钱。
  - **前端**：「升级会员」弹窗升级为「会员与充值」双页签——Token 充值页：余额卡+今日免费额度+
    充值包+收支明细；管理后台新增「Token 经济」页：总开关/倍率/汇率/免费额度/充值包管理+
    给指定用户手动加/减 token（查余额+最近流水）。
  - **安全设计**：总开关默认关（关=完全不计费不拦截，现网零影响）；管理员豁免；计费层任何异常
    fail-open 放行，绝不因计费故障把用户挡在门外。本期 token=单用途消费积分（不可退现），
    合规风险最小化；创作者商城/分成账本为下一片（P2）。
- 优化：本地开发支持全链路联调——`client/.env.local` 设 `VITE_HS_URL` + vite dev 代理
  /_matrix、/cosmac 到本机 Synapse/bot（线上零影响）；本地 run/synapse 配置修复目录改名后的
  旧绝对路径。
- 本周（1.5.28→1.6.0）另含：全项目图标统一换 Lucide 线性图标、多项 AI 行为/任务编排修复
  （虚假交付、卡死超时、内容归属、频道管理刷新等），详见各 patch 条目。
...
- 优化：「我的协作人」点「同步到平台」/「清除」后**页面弹回顶部，滚动位置丢失**(负责人实报)。原因是
  这两个操作内部会 `load()` 全量重拉、列表重渲染,scrollTop 归零。现包一层:操作前记下滚动位置、
  重渲染后 nextTick 恢复(复用弹窗里"进/出编辑保留滚动"的同套机制),停在原处不用重新找人。

## 2026-07-25 — GuDuu OS 1.5.39 (patch)

- 修复：**管理后台右上角「关闭」按钮丢失，无法返回主系统**(负责人实报)。根因是 1.5.7 给页头做吸顶
  (`.adm-head` position:sticky + z-index:5 + 全宽背景)后,层级高于关闭按钮(`.adm-close` z-index:3),
  页头背景把关闭按钮盖住了。把关闭按钮 z-index 提到 20(高于吸顶头部),恢复可见可点。

## 2026-07-25 — GuDuu OS 1.5.38 (patch)

- 修复：**多个 AI Agent 写的内容全被中枢AI 以自己(@guduu)名义统一发出，频道里分不清哪段是谁写的**
  (负责人实报)。工具层其实早支持"代同事署名发言"(`send_message_to_room` 的 `as_agent`:会先把该
  同事傀儡拉进频道、再以它身份发,失败才退回主AI)——问题是模型没用它、自作主张"由我统一发"。修:
  在**每轮硬注入的交互准则**里加一条硬规则:让 AI 同事写内容发频道时,**每段用 as_agent 以该同事
  身份分别发出**(各填各的 slug、分多次调用),绝不攒成一条以主AI 名义统一发。这样各段内容署名到
  对应 Agent、同事也会自动进频道。

## 2026-07-25 — GuDuu OS 1.5.37 (patch)

- 修复：**让 AI 更新真人任务状态，AI 回复"更新完成"但看板实际没变(虚假反馈)**(负责人实报)。根因:
  AI 的 `update_task` 越权闸死限"只能改**本频道**的任务",而被指派者(真人)在别的频道/私聊里让 AI 改
  自己的任务(如 #51)会被拦;AI 收到"没找到本频道的任务"后,竟**另建了个新任务(#58)标 done 并谎报
  #51 已完成**。修:① `update_task` 授权改为与任务看板「看得到=改得动」同口径——执行者本人/下达者/
  管理员/任务所属房间成员任一即可,不再死限本频道,执行者本人从任何地方都能推进自己的任务;
  ② 无权时明确告知模型"**不要另建新任务假装完成**",堵住虚假反馈。

## 2026-07-25 — GuDuu OS 1.5.36 (patch)

- 优化：**管理员在「我的协作人」设置能力，自动同步到后台「人员能力」平台名册**(负责人实报两处不同步;
  拍板"管理员设置即自动上平台")。原因:「我的协作人」是个人标注(按 owner 隔离)、后台「人员能力」是
  平台名册(cosmac.people),两套数据,原需手动点「提升为平台」才同步。现管理员在「我的协作人」设/改
  能力时,复用 promote 机制自动同步进平台名册,后台立刻一致;普通用户仍保持个人私有。平台同步失败
  不影响个人已保存。

## 2026-07-25 — GuDuu OS 1.5.35 (patch)

- 修复：后台在「频道管理」等菜单下**刷新浏览器，列表数据消失**(其它菜单同)(负责人实报)。
  根因:各菜单的数据是点菜单时(switchToX)才加载的,而**从深链刷新直接落在某菜单**时没人触发加载,
  check() 只加载了用户列表(此前只额外补了频道一个)。修:加统一的 `loadActiveTab(t)` 分发,
  管理员确认后按当前 tab 补加载一次(频道/技能/智能体/人员/模板/规则/工作流/门控/配额/套餐/归档/
  概览/平台知识库/页面内容全覆盖),用各自 loaded 标记兜底、不与后续点击重复拉。

## 2026-07-25 — GuDuu OS 1.5.34 (patch)

- 修复：**频道内 @AI 归档、点了确认选择卡后 AI 无响应**(负责人实报)。根因是点选择卡把选项当**普通
  消息**发回房间,但**频道里 AI 只在被 @ 时才回**——这条不带 @提及,频道 AI 收不到、就没有下一步
  (私聊里每条都交给 AI 所以之前没暴露)。修:选择卡点选时给消息带上对主 AI 的 `m.mentions`,
  bot 的 mention 判定命中、正常继续(如确认后执行归档)。私聊场景不受影响。

## 2026-07-25 — GuDuu OS 1.5.33 (patch)

- 修复：**在中枢AI私人会话里归档某频道，实际没归档成功**——左侧频道无归档图标、后台归档记录也没
  记录(负责人实报;DB 实证该频道无归档记录)。根因是 `archive_project` 只归档**当前房间**(ctx.room_id),
  私聊里 ctx.room_id 是私聊房、没有任务,工具返回"本频道没有任务",模型却谎报成功。**修**:给
  archive_project 加 `room` 参数(按频道名/ID 定位),私聊里归档必须指明频道(否则明确提示,绝不把
  私聊当频道归档);带成员越权校验 + 按**目标频道**校验归档权限。归档真正落到目标频道后,后台归档
  记录与左侧频道归档图标(客户端本就读 `cosmac.project.archived` state)都会正常出现。

## 2026-07-25 — GuDuu OS 1.5.32 (patch)

- 优化：AI Agent 商城获取资源**达上限时的提醒更醒目**(负责人实报:提醒不够明显,用户以为点了没反应)。
  原来达上限的错误和"使用指引"共用同一条不起眼的 💡 提示条。现把**获取失败(如达上限)**单独用
  **橙红警告条**(⚠️ + 左侧粗竖条 + 加粗 + 可关闭)展示,与 💡 使用指引明显区分,深色模式同款适配;
  点「获取」达上限会立刻弹出,不会再误以为没反应。

## 2026-07-25 — GuDuu OS 1.5.31 (patch)

- 修复：AI 邀请成员时**不校验成员状态与群内关系**——能邀**已停用账号**(登不进来)和**群主本人**
  (负责人实报)。现给邀请加前置校验:① 发起人/群主本人 → 拦下(已在群、无需邀自己);② 已是群成员
  → 拦下(重复邀请);③ 已停用账号 → 拦下(登不进来,邀了白搭)。单个邀请(invite_to_room)直接返回
  原因;建专班批量邀请把停用账号跳过并在开班消息/回灌里如实说明(不假装邀了)。停用账号集复用
  已有的 120s 缓存(与任务派单过滤同源)。

## 2026-07-25 — GuDuu OS 1.5.30 (patch)

- 优化：全屏中枢AI聊天右侧「进度」面板**改为跟随对话正在推进的项目**,不再按当前工作区过滤
  (负责人实报:AI 拆解的项目与右侧进度对不上)。根因是中枢AI 是跨工作区的全局助理,而进度面板
  却按 activeSpace 过滤——你在对话里拆的是 A 项目(在别的工作区),进度却显示当前工作区 B 项目。
  现「当前对话项目」= 最新创建任务所属的项目(goal),进度面板显示该项目的任务、并标出项目名,
  与左侧 AI 画的流程图一一对应。仅改 AI 面板,主任务看板仍按工作区(不受影响)。

## 2026-07-25 — GuDuu OS 1.5.29 (patch)

- 修复：AI 能建出**两个完全同名的频道/专班**(负责人实报,违反唯一性)。根因两处:① 建专班的
  防重名靠 Synapse `/joined_rooms` 列表,而它有**最终一致性延迟**——刚建的第一个还没进列表时又建
  一个,就检测不到重名、防重名失效;② 普通建频道工具 `create_room` **压根没做防重名**。修:Toolbox
  记一份本进程"刚建过的频道名",防重名把它并进来(补足 /joined_rooms 的延迟);并给 `create_room`
  也加上防重名(普通频道用中性数字后缀「（2）」,专班仍加「专班」后缀)。加连建同名的回归测试。
...
- 修复：任务看板截止时间徽章把**明天到期**的任务误标成"今天到期"(负责人实报)。根因是用"距现在
  不足 24 小时"判"今天"——今晚 22:00 看明早 09:00 的任务才隔 11 小时,就被当成今天。改为按**自然日
  (日历天)**比较:午夜对齐算天数差,今天/明天/还剩N天/日期截止各归各;并**新增「明天到期」**档
  (原来直接跳到"还剩N天",不够直观)。

## 2026-07-25 — GuDuu OS 1.5.26 (patch)

- 修复：AI 建专班"成功"后**任务看板不生成任何任务**(负责人实报;DB 实证该专班 0 任务)。根因是
  `assemble_team` 工具把任务分解(`tasks`)标为"可空",较弱的模型(DeepSeek)常直接漏传,于是建了个
  **空壳专班**——频道建好、看板却空。**修**:给 `assemble_team` 加**任务闸**——没带任务(或全是空标题)
  时**在建房/扣配额之前**就早退,明确要求模型先把目标拆成 3~8 个子任务再调,从根上杜绝空专班;
  schema 里 `tasks` 描述也从"可空"改为"必填(只想建群绑资源用 create_room)"。加无任务/空标题两个回归测试。

## 2026-07-25 — GuDuu OS 1.5.25 (patch)

- 修复：中枢 AI 列"当前账号下的频道"时，把**别的管理员工作区的频道**也列了出来(负责人实报,
  跨账号越界)。根因是 `list_my_rooms` 工具给平台管理员额外并入了 **bot 服务的全部频道**(跨所有
  账号/工作区),意图是"管理员跨工作区统筹",实则越出了"当前账号"的范围。**修**:作用域统一为
  **发起人自己真实加入的房间**(与其侧栏工作区完全一致),管理员亦然;回退口径同样按成员身份过滤、
  不给管理员开后门;标题不再自称"全部/跨工作区"。要平台级频道审计走后台「频道管理」页(全服视角的
  管理界面)。新增跨租户越界回归测试。

## 2026-07-25 — GuDuu OS 1.5.24 (patch)

- 修复：后台「数据概览」账号总数与「用户管理」不一致(负责人实报)。两处本用同一数据源,差异真因是
  **数据概览只首次加载、之后缓存**——后来增删/停用账号后再回来就与用户管理对不上。改为每次进入
  数据概览都重新拉取。
- 变更：**账号统计不再计入中枢AI**(@guduu 系统机器人,非真实用户;负责人拍板)。数据概览「账号总数」
  与用户管理「共 N 个账号 / 管理员 / 已停用 / 已注销」统一剔除中枢AI;用户管理列表仍展示它(带「中枢AI」
  标签、可管理)。两处口径彻底一致。

## 2026-07-25 — GuDuu OS 1.5.23 (patch)

- 修复：频道知识库上传部分 CSV 报「入库失败（数据库不可用？）」(负责人实报)。根因不是数据库
  不可用(同频道 .md 入库正常)——而是 Excel/记事本导出的 CSV 常带 **NUL(`\x00`) 或二进制残渣**,
  而 **Postgres 的 text 字段存不了 NUL**,INSERT 直接抛错,被一个 `except Exception` 兜底成了误导性的
  "数据库不可用"。**修**:入库引擎 `ingest_document` 在切块/落库前统一清洗 NUL 及非文本控制字符
  (保留 \t\n\r),标题同样清——任何来源(csv/上传/粘贴)都安全;前后端两处上传清洗(`cleanUploadText`
  / `clean_upload_text`)也同步加上,顺带让字数统计不被二进制虚高。

## 2026-07-25 — GuDuu OS 1.5.22 (patch)

- 修复：登录页从「邮箱登录」切到「账号登录」后，输入框仍回显上一个 Tab 输入的邮箱(负责人实报)。
  根因是 Vue 元素复用陷阱——账号框/邮箱框是同位置的 `v-if/v-else` 且未加 `key`,Vue 复用同一个
  DOM input,切 Tab 后旧值残留刷不掉。给两个输入框加不同 `key` 强制各建独立 input。本地浏览器
  实测:邮箱登录输入邮箱后切回账号登录,输入框已清空。

## 2026-07-25 — GuDuu OS 1.5.21 (patch)

- 优化：后台技能库把「自定义技能」区块挪到**页面顶部**、预置技能库下移(负责人建议)。原来自己
  建的技能在一长串预置技能列表**最底部**,每次要滚很久才看到。现顺序=编辑表单→自定义技能→
  预置技能库;顺带修正"上方/下方"指向文案与「已自定义」标签提示。与智能体页(本就自定义在前)一致。

## 2026-07-25 — GuDuu OS 1.5.20 (patch)

- 优化：知识库「我的文档」列表右侧样式统一(负责人实报"文字/图标混用")。原来文档行显示的是
  来源字段英文原值(`upload`),紧挨删除图标,像两个操作按钮。现把**来源本地化成中文**
  (上传/引导/平台/文档)并做成**低调胶囊标签**(明确是元信息而非操作),删除按钮也统一改成
  Lucide 线性图标(与全站一致)。

## 2026-07-25 — GuDuu OS 1.5.19 (patch)

- 修复：**AI Agent 执行完任务没更新看板、也没把初稿发到频道**(负责人实报)。查任务表定位:
  某专班的 agent 任务全卡在 `doing`/进度 0/无产出——而进度 0 是铁证(自动执行器一启动就会设
  进度 10),说明**自动执行根本没为这批任务跑**。根因:自动执行**只在 `assemble_team` 建任务时
  内联指定了 agent 才触发**;若任务是事后由中枢 AI 用 `update_task` **改派给 AI 同事**的
  (拆解时先建 todo、再补派执行者,弱模型很常见),就没人去执行→永远躺在看板、频道无初稿。
  **修**:`update_task` 把任务派/改派给 agent(未完成、尚无产出)时,补触发一次自动执行(产出→
  发频道→回填 done)。并加可观测性:自动执行线程起止/异常均记 INFO/exception 日志(此前成功走
  debug、崩溃只进 stderr,随容器重建即丢,事后无从查)。
...
- 修复：**换设备登录后，先前设备被登出、新设备却也登不上**(负责人实报)。查明两点：
  - ① **真实客户端 IP 没传到后端**——双层反代(宿主 Caddy→容器 Caddy→bot)下,容器只看到网桥
    网关 `172.18.0.1`,于是 Synapse 登录限频(rc_login)把**全平台登录当同一 IP 一起限流**,
    多人/多次登录很快触顶→误报「尝试过于频繁」把正常用户挡在门外(审计表实证:多个不同用户
    同时段 `hs_rate_limited`、IP 全是 172.18.0.1)。**修**:容器 Caddy 加 `trusted_proxies
    private_ranges` 从 XFF 还原真实客户端 IP,并以 `X-Real-IP` 传给 bot(两个 Caddy 模板都改);
    限频/异地检测/审计恢复按真实 IP 各算各的,顺带修好之前失效的撞库防护。
  - ② **单端在线(后登录踢先登录)默认改为多设备同时在线**(负责人拍板)。原为强制"后踢前",
    现改成 env 开关 `COSMAC_SINGLE_SESSION`(默认关=多设备,标准 Matrix 行为;置 1 才互踢)。
  - 注:容器 Caddyfile 是安装时渲染的一次性产物,`update.sh` 不重渲——本次已在生产手动补同款
    配置;模板改动惠及后续新装实例。

## 2026-07-25 — GuDuu OS 1.5.16 (patch)

- 变更：**图标换 Lucide 第二期——剩余弹窗/面板全部转完**(接 1.5.15,负责人验收风格后铺开)。
  已转:右侧面板(关于/人员/技能/知识库/规则各区块头)、频道管理弹窗(AI写规则/群主标/文档/
  上传/规则文档)、社媒数据源弹窗、入驻引导、会员升级弹窗、图文教程(前台阅读+频道编辑)、
  组织架构、AI 工坊、知识库弹窗、异地登录验证提示等。
  **刻意保留**:① 社媒平台图标(X/抖音/小红书等品牌标识,Lucide 无品牌图标,换成通用图标反而
  丢辨识度);② 入驻模板的默认图标(数据字段,后台可改);③ 正文 💡提示/⚠️警告/✅状态等强调符;
  ④ 终端/CLI 的提示符与假输出。至此界面结构性图标已统一为 Lucide 线性风格。

## 2026-07-25 — GuDuu OS 1.5.15 (minor)

- 变更：**全项目图标从彩色 emoji 换成 Lucide 线性图标**(负责人拍板;emoji 各平台渲染不一致、
  大小色彩不可控——之前多次"图标大小不一/风格不统一"的根因)。第一期(核心导航)已落地:
  - 新增统一图标出口 `components/Icon.vue`(基于 `lucide-vue-next`,tree-shake;模板只写
    `<Icon name="settings" />`,图标源与命名集中管理,以后换/加图标只改这一处);单色继承
    `currentColor`,深色模式自动适配、可按状态染色。
  - 已转:管理后台左侧菜单/详情区块/标签/搜索框放大镜、主界面左侧栏/账号菜单/应用九宫格/
    数据看板/任务看板/右侧面板等**约 110 处**。
  - 正文里的 💡提示/⚠️警告/✅状态等**强调性 emoji 暂保留**(按约定);其余弹窗/面板(频道管理弹窗、
    入驻引导、会员/工坊/市场等 ~16 个组件)将在下一期转完。

## 2026-07-25 — GuDuu OS 1.5.14 (patch)

- 优化：账号下拉菜单「管理后台」「退出登录」图标与上方各项统一大小(负责人实报)。原来这两项用的
  是普通 Unicode 符号(`⚙`/`⎋`),渲染成又细又小的单色字形,与上面的彩色 emoji(👤🎭📈🔒🔔)
  不一致;改为同风格 emoji(⚙️ 齿轮 / 🚪 门),大小、色彩一致。

## 2026-07-25 — GuDuu OS 1.5.13 (patch)

- 修复：**「用户管理」与「人员能力」两页对同一账号的状态显示不一致**(负责人实报)。根因:
  1.5.6 把账号状态拆成三态(停用=locked 可无损恢复 / 注销=deactivated 旧版已退所有频道),
  「用户管理」已更新,但「人员能力」页漏改——它只带了 `deactivated` 一个标记、还沿用旧文案把它
  显示成「已停用」,于是同一个**已注销**账号在用户管理显「已注销」、在人员能力显「已停用」。
  现让人员能力也带上 `locked`,按用户管理同一套三态渲染(注销/停用/休假中);停用与注销账号
  一并灰显、禁「设置能力」、排序沉底,两页彻底对齐。

## 2026-07-25 — GuDuu OS 1.5.12 (patch)

- 修复：**后台技能库·自定义技能「编辑」点击无反应**(负责人实报)。其实按钮有效——编辑表单
  渲染在内容区**顶部**,而列表里的「编辑/新建/覆盖为自定义」按钮在滚动后的下方,点了表单在
  视口上方打开、当前屏毫无变化,看着像没反应。现改为:任一编辑/新建表单打开时,内容区平滑
  滚回顶部让表单入眼。集中用一个 watcher 覆盖后台**六页**(技能/智能体/人员能力/入驻模板/
  会员套餐/工作流)结构相同的编辑表单,一次修全、避免同类问题在其它页复发。
...
- 修复：**任务看板右上角「刷新」点击无反应**(负责人实报)。根因两处:① 取任务列表的
  `getTasks` GET 没禁用 HTTP 缓存,URL 与 token 固定,浏览器把上次响应缓存了,点刷新拿回
  同一份旧数据、看板毫无变化——观感就是"按钮坏了"(与 1.5.8 后台缓存同一类问题)。已加
  `cache: 'no-store'`,每次都走网络拿最新任务。② 刷新按钮点下去没有任何视觉反馈,即便真刷新
  了、数据没变也像"没反应"。现给按钮加转圈+禁用态(请求过快也至少转 ~400ms),让点击有明确回应。

## 2026-07-25 — GuDuu OS 1.5.9 (patch)

- 优化：**后台「用户管理 / 频道管理」列表按创建时间倒序,新增的排最上**(负责人要求,配合
  1.5.8 的"新建即可见")。用户列表用 Synapse admin API 原生 `order_by=creation_ts&dir=b`
  拿倒序(在用账号新→旧、已停用沉底);频道的 admin API **不支持按创建时间排序**,改由后台
  判房型那个 bot 端点(`room_kinds`)同一趟读 state 时顺手取 `m.room.create` 的
  `origin_server_ts` 带回(零额外请求),前端按它倒序(拿不到时间的沉底、退回按成员数稳定排)。
  顺带修正:旧版判到房型标记就 `break`,普通频道走不到 create 行——现改为先记建房时间再归类。

## 2026-07-25 — GuDuu OS 1.5.8 (patch)

- 修复：**新建频道后,管理后台频道列表不刷新,需硬刷新浏览器才看到**(负责人实报)。
  根因两处:① 后台所有 admin 接口的 GET(频道/用户列表等)URL 固定不变,浏览器按启发式
  缓存把旧响应直接还给页面——连点页面内「刷新」拿到的也是缓存旧数据,只有硬刷新浏览器
  才会重新校验。现给 `adminFetch` 加 `cache: 'no-store'`,每次都走网络、绝不读写 HTTP 缓存
  (顺带修好用户列表等所有后台页的实时性)。② 同一次后台会话里离开频道页再回来,`roomsLoaded`
  首次缓存会跳过重新拉取——改为每次进入频道 tab 都重新拉;并补上"直接从地址打开/重开时
  正好停在频道页"也自动加载。

## 2026-07-25 — GuDuu OS 1.5.7 (patch)

- 优化：后台管理各页**顶部信息区固定吸顶 + 右侧常显滚动条**(负责人实报:用户管理/频道
  管理/归档记录/技能库/智能体/人员能力/工作流/入驻模板等页,长列表往下滚时页头、统计、
  搜索/筛选栏都随内容滚走,看不出"这是哪一页、共多少条、搜索框在哪")。改为内容区自身
  滚动、页头(标题+统计+操作)与筛选栏 `sticky` 吸顶;滚动条常显(webkit 半透明圆角、
  `scrollbar-gutter: stable` 预留槽位避免内容左右跳动)。页头高度会随窗口变窄、统计文案
  换行而变高,写死数值窄屏会让筛选栏盖住统计行——改由 `ResizeObserver` 实测页头高度写进
  CSS 变量 `--adm-head-h`,筛选栏的 `top` 引用它,换页时自动重挂观察器。

## 2026-07-24 — GuDuu OS 1.5.6 (patch)

- 修复：后台用户管理筛「已停用」却列出「已注销」的账号(负责人实报)——状态列本已区分
  三态(正常/已停用 locked/已注销 deactivated),但**筛选器只有一个「已停用」选项**且谓词
  用 `deactivated || locked` 混筛,顶部统计同样混成一个数。两种状态的恢复方式完全不同
  (停用=无损恢复;注销=旧版恢复且需重新入群),混在一起管理员会点错按钮。现筛选器拆成
  「已停用 / 已注销」两项、各自精确匹配,顶部统计也分开报。

## 2026-07-24 — GuDuu OS 1.5.5 (patch)

- 修复：**管理员停用账号后,该账号从用户列表直接消失**(负责人实报;生产实测已复现)——
  Synapse 的 /admin/v2/users **默认排除 locked 用户**,需显式带 `locked=true`;而本产品的
  「停用」正是用 lock 实现的(当初改锁定是为保住频道/工作区成员关系、可恢复)。查询漏了
  这个参数,后果有两处:①后台用户列表看不到被停用的人,既看不到状态**也无法恢复**;
  ②后端「停用者名单」(AI 派单过滤用)同样漏掉他们 → 任务被派给登不进来的账号。
  前后端两处查询均补 `locked=true`,生产实测:锁定后旧参数查不到、新参数正常返回且
  状态字段正确。新增回归测试锁死该参数。

## 2026-07-24 — GuDuu OS 1.5.4 (patch)

- 优化：频道管理标题里频道名与「· 群级 AI 隔离配置」贴在一起(HTML 折叠了标签边界的
  前导空格)——改用 margin-left 留白。1.4.9 标题改造的收尾细节。

## 2026-07-24 — GuDuu OS 1.5.3 (patch)

- 修复：AI 查频道成员漏掉「待接受」的人(负责人实报:前端人员列表 5 人含一个待接受,
  AI 只报 4 人,还回答"工具返回里没有邀请状态字段,我判断不了")——成员查询走的是
  /joined_members,只含**已加入**。现改读 m.room.member state:**已加入 + 待接受分组
  返回**并标注状态,工具说明也要求 AI"讲清楚谁还没接受、别把待接受的人当在场成员派活
  或计入人数"。老部署/接口异常自动回退已加入列表,绝不因此答不出成员。

## 2026-07-24 — GuDuu OS 1.5.2 (patch)

- 优化：同一发送者连发多条时上下内容分不清(负责人实报:中枢AI 连发「执行过程卡→@提醒
  →长帖子」糊成一片,要鼠标移上去才分得清)——折叠态消息之间加 8px 间距 + 一条淡分隔线
  (从消息体起始处画起、不跨头像槽,悬停/未读时自动弱化避免与背景打架)。本地做了改前/改后
  对比页实测,边界一眼可见。

## 2026-07-24 — GuDuu OS 1.5.1 (patch)

- 修复：主 AI 代 AI 同事写的内容全部署名主AI、看不出谁写的(负责人实报"之前 Agent 做完
  自己发")——两条链路不一致:**任务自动执行**一直以傀儡身份发(频道里显示「文案」在说话,
  能力从未退化);但主 AI 在**对话里代写**时,发消息工具没有"以谁的身份发"的选项,只能
  署名 @guduu。现给 send_message_to_room 加 `as_agent` 参数(填 Agent slug):拉傀儡进房→
  以其身份发,频道里即显示该同事本人;工具说明明确要求"多个 Agent 各写一段就分多次调用、
  各署各的名"。傀儡不可用/发送失败**退回主AI身份、绝不丢消息**,且返回文案如实说明实际
  署名(防模型宣称"已以 XX 名义发出"而其实没有)。
- 附:该频道三个 Agent 傀儡本就不在群里(非 assemble_team 建的专班),as_agent 会按需拉进。

## 2026-07-24 — GuDuu OS 1.5.0 (minor)

- 新增(负责人拍板 A+B,解「协作人能力设置后没同步」)：前台「我的协作人」(个人级、按 owner
  隔离)与后台「人员能力」(平台级、控制室 cosmac.people)此前两套数据互不可见,管理员在后台
  看不到用户标注了谁擅长什么(虽然 AI 派单一直合并两份)。现打通:
  · **A 后台可见**:「人员能力」表新增「个人标注」列——只读展示 谁给 TA 标了什么(N 人标注
    + 摘要,悬浮看全部),数据归属不变(仍是个人私有标注)。新端点 /cosmac/admin/people_notes
    (仅管理员,聚合上限 2000 条)。
  · **B 一键同步**:「我的协作人」里管理员可对自己标注的条目点「同步到平台」,写进平台名册
    全平台可见。幂等——同 user_id 覆盖、其余条目原样保留;无控制室写权限时如实报错不假装成功。
    新端点 /cosmac/people/promote(仅管理员)。
- 排查附记:本次报的"设了看不到"经实测**数据与接口均正常**(拿本人 token 调 people/mine 返回
  4 条个人标注),截图应为强刷前的缓存页;真问题是上述两套数据不互通,已按 A+B 解决。

## 2026-07-24 — GuDuu OS 1.4.9 (patch)

- 优化：频道管理弹窗标题只显示 room_id、看不出是哪个频道(负责人建议:ID 前加频道名,
  并考虑长名折行)——改为两行:第一行「**频道名** · 群级 AI 隔离配置」、第二行 room_id
  (等宽小字、可整段选中复制)。长频道名单行省略号 + 悬浮 title 看全名,不折行撑破标题栏;
  弹窗内其他引用频道名处(如数据权限提示)也随之显示真名而非 id。
