              ...
              <tbody>
                <tr v-for="a in presetAgentsFiltered" :key="'pa-' + a.slug">
                  ...
                  <td class="adm-row-actions">
                    <button
                      class="adm-btn ghost sm"
                      :disabled="agSaving || agOverriddenSlugs.has(a.slug)"
                      :title="agOverriddenSlugs.has(a.slug)
                        ? '已经有一份同标识的自定义版在生效，覆盖内置的'
                        : '内置智能体改不动。点它把这条复制成一份可编辑的自定义副本（同标识 slug），你改完保存后就用你这份、盖掉内置的。'"
                      @click="overridePresetAgent(a)"
                    >
      ...
      <template v-else-if="tab === 'gating'">
        <header class="adm-head">
          <div>
            ...
            <p class="adm-hint">
              这里配的是<b>使用某功能所需的最低会员等级</b>：选「免费可用」= 所有人能用；选「付费会员及以上」= 免费用户不能用。
              · 主 AI 在服务端强制(客户端只是配置) · 保存后约 15 秒热生效<br />
              要限制「能用多少」(次数/篇数/空间等数量上限,按等级填数字)，请到左侧「📈 用量配额」页配置。
            </p>
          </div>
          <div class="adm-actions">
        ...
        <div v-else class="adm-form">
          <p class="adm-hint">
            「免费可用」= 使用门槛为免费级,人人可用(数量上限另在「用量配额」页按等级配);
            选更高等级则低于该等级的用户被挡并提示升级。
          ...
          <template v-else>
            <div v-for="m in memberList" :key="m" class="adm-mrow">
              ...
              <span class="adm-mid">
                ...
                <template v-if="isBotMember(m)"><b :title="`真实账号:${m}`">@{{ instanceBrand.productName }}:{{ m.split(':')[1] || '' }}</b></template>
...
async function doDeleteRoom(r: AdminRoom) {
  ...
  if (isCtrlRoom(r)) {
    warn('不可删除', `这是 ${instanceBrand.productName} 控制室，删除会清空全部平台配置，已阻止。`)
    return
  }
...
function displayCreator(uid: string): string {
  if (uid.startsWith('@guduu:')) return `@${instanceBrand.productName}:` + uid.split(':')[1]
  return uid
}
