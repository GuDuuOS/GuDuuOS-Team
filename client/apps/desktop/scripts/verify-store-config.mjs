...
const files = {
  ...
  desktopServers: await readFile(path.join(desktopRoot, 'desktop-server-discovery.mjs'), 'utf8'),
...
const assertions = [
  ...
  [files.preload.includes('servers: Object.freeze'), '逐项命名服务器发现桥'],
  [files.main.includes('installDesktopServerDiscoveryIpc()'), '服务器发现 IPC 校验入口'],
  [files.desktopServers.includes("candidate.protocol !== 'https:'"), 'OEM 发现生产强制 HTTPS'],
  [files.desktopServers.includes('MAX_INSTANCE_CONFIG_BYTES'), 'OEM 公开配置大小上限'],
  [!files.desktopServers.includes('accessToken'), 'OEM 发现不接触 access token'],
