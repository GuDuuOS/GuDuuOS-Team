...
**模块6当前增量（2026-07-31）**：建设 Nexus **版本发布中心**。新版本默认未发布，
超级管理员维护永久历史版本列表，发布流程为“未发布→灰度监测→全量发布→暂停”，
并可从历史列表选择已发布旧版本向全节点回撤；节点通过宿主更新代理按 KEY 拉取指令
并上报 pending/installing/success/failed。全 OEM 数据大屏只允许超管登录后兑换只读
令牌进入。Nexus 不主动 SSH OEM
