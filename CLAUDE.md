...
**模块6当前增量（2026-07-31）**：完善 Nexus 超级管理员工作台。后台采用左侧功能导航，
把舰队总览、版本管理、节点实例、授权申请、OEM 客户、支付订单和只读数据大屏分区
呈现；新版本默认未发布，历史列表可选择已发布旧版本向全节点回撤。节点通过宿主更新
代理按 KEY 拉取指令并上报 pending/installing/success/failed。Nexus 不主动 SSH OEM
