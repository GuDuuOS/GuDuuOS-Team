...
- “GuDuu OS”与“中富通”为官网正式品牌，只有 Nexus 激活实例 #3
  可作为节点产品名、公司标识和官网联系信息使用。节点 #1、#2 只承担内部开发与灰度，
  仍按 OEM 品牌边界配置自己的名称与 Logo；其他 OEM 同样必须使用自有品牌。后端必须根据
  服务器持久化的激活实例号强制校验，禁止只靠前端隐藏或可篡改字段判断。
  实例号只能来自 Nexus 授权兑换或已用 OEM KEY 验证的心跳响应，必须在新安装
  时直接落盘，存量节点缺失时由成功心跳自动修复，不允许由浏览器自报节点编号。
...
> **Nexus 节点余额与财务清单（2026-08）**：节点实例页的 Token 充值必须使用
> 明确的常用额度选择和带千分位的精确数字输入，提交前同时展示当前余额、充值额度与
> 充值后余额，避免把“万/亿”口语换算错误。所有人工充值与支付成功后的 Token 入账
> 继续共用 ``nexus_ledger`` 这一份余额真相；财务页按流水关联订单后区分“人工入账”与
> “支付入账”，展示节点、企业、Token、实收金额、渠道、操作人/订单号和时间。人工充值
> 没有支付订单时金额必须显示为“无支付单”，不得推算或伪造收入；既有流水按备注中的
> 历史订单号兼容识别，新人工流水须写入具名管理员和备注用于审计。
...
> **OEM 官网与应用同镜像（2026-08）**：节点 `web` 镜像同时交付公开官网与登录后的
> 协作应用，域名根路径展示官网，工作台使用独立应用路由；禁止再复制一套网站工程或增加
> 第三个运行镜像。官网名称与 Logo 复用节点品牌配置，主标题、介绍、联系方式、帮助与
> 隐私链接、页脚文案由节点“系统设置”统一维护，并通过只读公开配置端点下发；公开端点
> 不得包含 SMTP、AI、支付等密钥。所有非正式官网节点的可见文案同样执行保留品牌校验；
> 只有官网正式节点 #3 可以出现 `GuDuu OS`、中富通名称、股票代码及官方联系方式。
...
> 验收前保持 ``draft/development/canary``；归档不会创建节点任务，也不会向 OEM 重发旧公告。

> **Nexus 发布草稿自动化（2026-08）**：受信 CI 上传带不可变镜像 digest
> 的发行清单时，Nexus 同一事务内为该版本创建 `node + container`
> 未发布草稿，标题和说明从当前 `DEVLOG.md` 对应版本抽取，并自动先给固定开发
> 节点创建唯一安装任务。重放相同版本与清单必须幂等，不得覆盖管理员已编辑的
> 草稿或重复建任务；正式生产发布和回撤仍必须由具名管理员人工确认。

> **Nexus 节点环境与灰度门禁（2026-08）**：节点 #1 是负责人开发节点，
> 每次“节点运行代码发生变化且确实构建了新镜像”后最先收到任务并自动安装；
> 只有节点 #1 上报成功，Nexus 才自动把同一份 digest 分配给节点 #2，并由 #2
> 的宿主代理自动安装，无需技术人员再次点击同意。节点 #2 是公司技术灰度节点；
> 节点 #3 是唯一官网正式节点，保留 `GuDuu OS` 与中富通品牌，尚未部署时发布必须明确
> 失败，部署后也只有节点 #2 上报安装成功，超管才能人工把同一份 digest 发布给 #3。
...
> 选择“立即更新”后才拉取镜像；允许选择稍后处理或忽略当前版本。固定内部灰度节点 #2
> 在节点 #1 成功后自动安装同一候选摘要，不再等待人工批准；节点 #3 和其他生产 OEM
> 仍缺省关闭自动安装，其中 #3 只能由超管在灰度成功后人工发布。任何模式都保留摘要校验、
> 数据库备份、健康检查和失败自动回撤；Nexus 禁止主动 SSH，也禁止强制升级客户节点。
> 网页批准文件所在共享目录由宿主更新代理负责创建和维护权限；bot 容器只能原子写入批准
> 文件，不得对宿主目录执行 `chmod/chown`。仍运行 1.29.2/1.30.0、因旧逻辑卡住的节点，
> 可由服务器 root 显式运行宿主工具迁移的 `--approve-current`，该参数只能批准代理已经
> 校验并缓存的当前 release_id，不能接收任意版本、镜像或命令。
