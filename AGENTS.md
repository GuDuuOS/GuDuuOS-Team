...
> 不得包含 SMTP、AI、支付等密钥。外部 OEM 的官网全部可见文案同样执行保留品牌校验，
> 只有内部节点 #1/#2/#3 可以出现 `GuDuu OS` 字样。
...
> 每次“节点运行代码发生变化且确实构建了新镜像”后最先收到任务，并在宿主明确
> 配置的自动更新策略下直接安装；只有节点 #1 上报成功，Nexus 才自动把同一份
> digest 作为可选候选通知节点 #2。节点 #2 是公司技术灰度节点，仍由技术人员
> 确认安装和验收；节点 #3 是官网正式节点，尚未部署时发布必须明确失败，部署后
> 也只有节点 #2 上报安装成功，超管才能人工把同一份 digest 发布给 #3。
...
节点 #1 成功后再把同一摘要自动交给节点 #2 内部灰度，节点 #2 成功后才允许超管人工
发布节点 #3 与其他生产 OEM。仅修改 `nexus/`、Nexus 门户/大屏或中央平台运维配置时，
