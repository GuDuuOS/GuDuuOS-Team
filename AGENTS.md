...
> 未发布草稿，标题和说明从当前 `DEVLOG.md` 对应版本抽取。重放相同版本
> 与清单必须幂等，不得覆盖管理员已编辑的草稿；自动化只建草稿，
> 灰度、全量和回撤仍必须由具名管理员人工确认。
...
> GHCR，两条路径都必须校验相同内容 digest。对外只暴露 TLS 拉取端点；
> 同步写入账号只留在 Nexus 宿主 root 权限文件，OEM 节点只保存可撤销的
> 只读拉取凭据。自建仓不是 GHCR 的唯一替代；任一单仓故障都不得阻断已批准发布。
