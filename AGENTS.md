...
| 5 | 个人主页 | ⬜ | 需要客户端 UI 配合 |
| 6 | **OEM 体系（GuDuu Nexus + 发行版）** | 🟡 进行中 | 当前增量：建设 Nexus **版本发布中心**。新版本默认“未发布”，超级管理员维护永久历史版本列表，发布流程为“未发布→灰度监测→全量发布→暂停”，并可选择已发布过的旧版本发起全节点回撤；节点通过宿主更新代理按 KEY 拉取指令并上报 pending/installing/success/failed，禁止 Nexus 主动 SSH OEM 服务器，也禁止给 bot 容器宿主机控制权。大屏包含全 OEM 聚合数据，只允许超管登录后兑换只读令牌进入。 |
